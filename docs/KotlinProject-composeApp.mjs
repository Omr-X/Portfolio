...
const wasmFilePath = './KotlinProject-composeApp.wasm';
...
try {
  ...
  if (isBrowser) {
    wasmInstance = (await WebAssembly.instantiateStreaming(fetch(new URL('./KotlinProject-composeApp.wasm',import.meta.url).href), importObject, wasmOptions)).instance;
  }
} catch (e) {
